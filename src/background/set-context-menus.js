...
export default function setContextMenu (config) {
  if (window.contextMenuQueue) {
    window.contextMenuQueue = window.contextMenuQueue.then(executor)
  } else {
    window.contextMenuQueue = new Promise(executor)
  }

  function executor (allResolve) {
