...
import { updateActiveConfig, initConfig } from '@/_helpers/config-manager'

const req = require['context']('@/components/dictionaries', true, /\.scss$/)
req.keys().forEach(req)

initConfig().then(activeConfig => {
  const config: AppConfigMutable = JSON.parse(JSON.stringify(activeConfig))
