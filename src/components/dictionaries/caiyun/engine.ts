...
export const search: SearchFunction<CaiyunSearchResult, MachineTranslatePayload> = async (
  ...
  let tl: string = payload.tl || (
    options.tl === 'default'
      ? config.langCode.startsWith('zh') ? 'zh' : 'en'
      : options.tl
  )
...
function handleJSON (
  ...
  return {
    result: {
      ...
      trans: {
        text: trans,
        audio: `http://tts.baidu.com/text2audio?lan=zh&ie=UTF-8&spd=5&text=${encodeURIComponent(trans)}`
      },
      searchText: {
        text,
...
async function getToken (): Promise<string> {
  let { dict_caiyun } = await storage.local.get<{'dict_caiyun': CaiyunStorage}>('dict_caiyun')
  if (!dict_caiyun || (Date.now() - dict_caiyun.tokenDate > 15 * 60000)) {
    let token = 'token:3975l6lr5pcbvidl6jl2'
    try {
      const homepage = await fetch('https://fanyi.caiyunapp.com', { credentials: 'omit' })
        .then(r => r.text())

      const appjsPath = (homepage.match(/\/static\/js\/app\.\w+\.js/) || [''])[0]
      if (appjsPath) {
        const appjs = await fetch('https://fanyi.caiyunapp.com' + appjsPath).then(r => r.text())
